...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@ribajosmani", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo Maked by [Ribaj Osmani](https://t.me/ribajosmani)",
...
async def logo_(event):
    ...
    if not bg_:
        pics = []
        async for i in ubot2.iter_messages(
            "@ribaajosmani", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo Designed by [Ribaj Osmani Global](https://t.me/meribaj)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
...
__help__ = """ This is help menu for logomaker Logo Maker By 
                  [Ribaj Osmani](https://t.me/ribajosmani)
